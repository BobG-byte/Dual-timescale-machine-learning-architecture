function [ICA_data] = IC_SoC(cell_NASA ,data_type,lower,higher)
% clc,clear,close all
% load('B0005.mat');
% load('B0006.mat');
% load("B0007.mat");
...
for m=1:cycle_num
       ...
       for k=2:length(ICA_data(m).Voltage_measured)
           ...
           if sum_V>=deltaV && ICA_data(m).Voltage_measured(k)<higher && ICA_data(m).Voltage_measured(k)>lower
               sum_V=0;
...
for num = 1:cycle_num
    if  mod(num, 5) == 0 
        color = num / cycle_num;
        plot(ax, ICA_data(num).SoC_sequence, ICA_data(num).ICA_Gaussian, 'Color', cmap(num,:));
        hold on;
    end
end
...
for num = 1:cycle_num
    if num ~= 1 && num ~= 170 && mod(num, 1) == 0
        color = num / cycle_num;
        plot3(ax, ICA_data(num).Voltage_sequence, ICA_data(num).SoC_sequence, ICA_data(num).ICA_Gaussian, 'Color', cmap(num,:));
        hold on;
    end
end
